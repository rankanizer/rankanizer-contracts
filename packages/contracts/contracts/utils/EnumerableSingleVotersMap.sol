 ...
 * ```
 */
library EnumerableSingleVotersMap {
    ...
    struct Map {
        ...
        mapping(address => SingleVoter) _values;
    }
     ...
     */
    function at(Map storage map, uint256 index) internal view returns (address, SingleVoter storage) {
     ...
     */
    function get(Map storage map, address key) internal view returns (SingleVoter storage) {
     ...
     * @dev Returns the value associated with `key`.  O(1).
     */
    function getUnchecked(Map storage map, address key) internal view returns (SingleVoter storage) {
        return map._values[key];
    }
}
