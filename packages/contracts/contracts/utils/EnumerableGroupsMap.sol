...
library EnumerableGroupsMap {
     ...
     */
    function at(Map storage map, uint256 index) internal view returns (address, Group storage) {
     ...
     */
    function get(Map storage map, address key) internal view returns (Group storage) {
     ...
     * @dev Returns the value associated with `key`.  O(1).
     */
    function getUnchecked(Map storage map, address key) internal view returns (Group storage) {
        return map._values[key];
    }
}
