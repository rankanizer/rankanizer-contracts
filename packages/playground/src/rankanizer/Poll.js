...
class Poll extends Component {
  ...
  createPoll() {
    ...
    let candidates_list = []
    let n = parseInt(this.candidates.value)
    let i
    for (i = 1; i <= n; i++) {
      candidates_list[i-1] = i.toString();
    }
    console.log(this.props.ballot)
    //this.setState({loading: true});
    this.props.ballot.methods
        .initialize(candidates_list, this.duration.value.toString())
        ...
        >
            <div style={{boardspacing: '0 1em'}}>
                ...
                <div className='input-group mb-1'>
                    <input ref={(input)=> {this.candidates = input}} type='text' placeholder='Number of candidates' required/>
                </div>
